...
!function($d) {
  ...
  var empty = '\u00a0'; // NBSP
  var fgBorderStyle = '1px dotted rgba(0,0,0,0)';
  ...
  var creFgCharElm = function(start) {
    return util.createElement('span', {
      ...
      style : {
        ...
        borderBottom : fgBorderStyle,
        borderLeft : start? fgBorderStyle : '',
        borderRight : fgBorderStyle
      } });
  };
  ...
  var creBgCharElm = function(start) {
    return util.createElement('span', {
      ...
      style : {
        ...
        borderLeft : start? bgBorderStyle : '',
  ...
  var createInput = function(opts) {
    ...
    opts = util.extend({
        ...
        sectionRe : '\\d+',
        directions : {},
        getNumCharsInMonospace : function() { return 1; }
      }, opts);
    ...
    opts.mbformat = function() {
      ...
      for (var i = 0; i < opts.format.length; i += 1) {
        ...
        text += c;
        var w = opts.getNumCharsInMonospace(c);
        len.push(w);
        for (var j = 1; j < w; j += 1) {
          text += c;
          len.push(0);
    ...
    !function() {
      ...
      sections.forEach(function(section) {
        ...
        for (var i = section.start; i < section.end; i += 1) {
          var elm = creFgCharElm(i == section.start);
          fgElms.push(elm);
          charElms[i] = elm;
          bgElms.push(creBgCharElm(i == section.start) );
        }
        start = section.end;
      });
      // and rest.
